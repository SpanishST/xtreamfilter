...
# NOTE: /downloads is intentionally excluded — it may be an SMB/NFS mount
# where chown would fail or is not appropriate. The user is responsible for
# ensuring the download directory is writable by PUID/PGID.
if chown -R "$PUID:$PGID" /data 2>/tmp/chown_err; then
    ...
    if ! su -s /bin/sh appuser -c "test -w /data && test -w /downloads" 2>/dev/null; then
...
echo "[entrypoint] /data is owned and managed by the entrypoint"
echo "[entrypoint] /downloads is NOT managed — ensure it is writable by UID $PUID:$PGID (especially for SMB/NFS mounts)"
